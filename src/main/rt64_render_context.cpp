...
RT64::UserConfiguration::Antialiasing compute_max_supported_aa(RenderSampleCounts bits) {
    if (bits & RenderSampleCount::COUNT_2) {
        if (bits & RenderSampleCount::COUNT_4) {
            if (bits & RenderSampleCount::COUNT_8) {
...
zelda64::renderer::RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool debug) {
    ...
    if (app->device->getCapabilities().sampleLocations) {
        RenderSampleCounts color_sample_counts = app->device->getSampleCountsSupported(RenderFormat::R8G8B8A8_UNORM);
        RenderSampleCounts depth_sample_counts = app->device->getSampleCountsSupported(RenderFormat::D32_FLOAT);
        RenderSampleCounts common_sample_counts = color_sample_counts & depth_sample_counts;
